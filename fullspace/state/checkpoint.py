...
and time-travel inspection (read the state at any past step, or branch from it).
"""
...
class SqliteCheckpointer(Checkpointer):
    ...
    def get(self, thread_id: str, checkpoint_id: Optional[str] = None) -> Optional[Checkpoint]:
        if checkpoint_id is not None:
            ...
            cur = self._conn.execute(
                "SELECT * FROM checkpoints WHERE thread_id=? ORDER BY step DESC, checkpoint_id DESC LIMIT 1",
                (thread_id,),
            )
        row = cur.fetchone()
    ...
    def list(self, thread_id: str) -> list[Checkpoint]:
        cur = self._conn.execute(
            "SELECT * FROM checkpoints WHERE thread_id=? ORDER BY step ASC, checkpoint_id ASC",
            (thread_id,),
        )
        return [self._row_to_cp(r) for r in cur.fetchall()]
